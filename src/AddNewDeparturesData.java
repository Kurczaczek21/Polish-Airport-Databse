...
public class AddNewDeparturesData {
    ...
    public void appendDeparturesFromAirport(String airportName) throws Exception {
        ...
        try {
            workbook = new XSSFWorkbook(new FileInputStream("DataBase\\"+airportName+"_Departures.xlsx"));
            logger.info("File "+airportName+"_Departures.xlsxsuccessfully opened.");
        ...
        for (String ele : mainData) {
            ...
            if(check==lastRowArray.length){
                found = true;
                logger.error("Founded last flight in database.");       // sssssssssss
                continue;
            }
            if(!found){
                continue;
        ...
        if(!found){
            logger.error("FLIGHT NOT FOUNDED, adding whole database.");
            for (String ele : mainData) {
                if (ele.contains(",")) {
                    date = ele;
                    continue;
        ...
        try (FileOutputStream outputStream = new FileOutputStream("DataBase\\"+airportName+"_Departures.xlsx")) { //Flights_data_KRK_MAIN.xlsx
            workbook.write(outputStream);
            outputStream.close();
            logger.info("Excel file '"+airportName+"_Departures.xlsx' updated successfully.");
            logger.info("Added " + String.valueOf(index-start)+" new flights to "+airportName+" airport Departures database."  );
